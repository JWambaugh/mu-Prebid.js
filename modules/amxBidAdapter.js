...
function getID(loc) {
  ...
  const short = host
    .slice(host.length - (SIMPLE_TLD_TEST.test(loc.host) ? 3 : 2))
    .join('.')
...
function transformXmlSimple(bid) {
  ...
  _each(
    [bid.nurl].concat(
      bid.ext != null && bid.ext.himp != null ? bid.ext.himp : []
    ),
    pixel => {
      if (pixel != null) {
  ...
  return (
    bid.adm.slice(0, impressionIndex) +
    pixels.join('') +
    bid.adm.slice(impressionIndex)
  )
}
...
function decorateVideoADM(bid) {
  ...
  const pixels = [bid.nurl]
    .concat(bid.ext != null && bid.ext.himp != null ? bid.ext.himp : [])
    .filter(url => url != null)
  ...
  return (
    (dtdMatch != null ? dtdMatch[0] : generateDTD(doc)) +
    getOuterHTML(doc.documentElement)
  )
}
    ...
    return flatMap(Object.keys(response.r), bidID => {
      return flatMap(response.r[bidID], siteBid =>
        siteBid.b.map(bid => {
          ...
          // let ad = null;
          let ad =
            mediaType === BANNER ? decorateADM(bid) : decorateVideoADM(bid)
          if (ad == null) {
            return null
          }
